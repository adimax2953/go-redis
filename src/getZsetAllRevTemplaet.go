...
func (s *MyScriptor) GetZsetAllRev(keys, args []string) (*[]RedisResult, error) {
	res, err := s.Scriptor.ExecSha(GetZsetAllRevID, keys, args)
	if err != nil {
		logtool.LogError("GetZsetAllRev ExecSha Error", err)
		return nil, err
	}
	...
	for i := 0; i < count; i++ {
		...
		result[i] = *r
		if err != nil {
			logtool.LogError("GetZsetAllRev Value Error", err)
		}
	}
...
const (
	...
	--[[
		...
		local sender                                        = "GetZsetAllRev.lua"
		...
		if DBKey and ProjectKey and TagKey and k1 then
			...
			local tmp = redis.call('zrevrange',MAIN_KEY , k2, k3)
