...
func (s *MyScriptor) IncValueBatch(keys []string, args ...interface{}) (*[]RedisResult, error) {
	...
	res, err := s.Scriptor.ExecSha(IncValueBatchID, keys, values)
	if err != nil {
		logtool.LogError("IncValueBatch ExecSha Error", err)
		return nil, err
	}
	...
	for i := 0; i < count; i++ {
		...
		result[i] = *r
		if err != nil {
			logtool.LogError("IncValueBatch Value Error", err)
		}
	}
...
const (
	...
	IncValueBatchTemplate = `
	--[[
		Author      :   Adimax.Tsai
		Description :   IncValueBatch
