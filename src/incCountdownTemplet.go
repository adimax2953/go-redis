...
func (s *MyScriptor) IncCountDown(keys, args []string) (int64, int64) {
	res, err := s.Scriptor.ExecSha(IncCountDownID, keys, args)
	if err != nil {
		logtool.LogError("IncCountDown ExecSha Error", err)
		return 0, 0
	}
	result := &RedisResult{}
	...
	if err != nil {
		logtool.LogError("IncCountDown Error", err.Error())
	}
...
const (
	...
	IncCountDownTemplate = `
	--[[
		Author      :   Adimax.Tsai
		Description :   IncCountDown
